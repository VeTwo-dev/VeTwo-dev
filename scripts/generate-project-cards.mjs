...
import { getLocalThumbnailPath, getPlaceholderPath } from "./lib/thumbnails.mjs";
...
async function main() {
  ...
  try {
    ...
    for (const repo of latest) {
      // Use local thumbnail generated by generate-project-thumbnails.mjs (or fallback to placeholder)
      const { stat } = await import("node:fs/promises");
      const localThumb = getLocalThumbnailPath(repo.name, "svg");
      const pngThumb = getLocalThumbnailPath(repo.name, "png");
      let finalThumb = getPlaceholderPath();
      try {
        await stat(path.join(root, localThumb.replace("./", "")));
        finalThumb = localThumb;
        console.log(`[projects] Thumbnail for ${repo.name}: ${finalThumb} (local SVG)`);
      } catch {
        try {
          await stat(path.join(root, pngThumb.replace("./", "")));
          finalThumb = pngThumb;
          console.log(`[projects] Thumbnail for ${repo.name}: ${finalThumb} (local PNG)`);
        } catch {
          // Check for any existing thumbnail via thumbnails lib (downloaded)
          const { localThumbnailExists } = await import("./lib/thumbnails.mjs");
          const existing = await localThumbnailExists(repo.name);
          if (existing) {
            finalThumb = existing;
            console.log(`[projects] Thumbnail for ${repo.name}: ${finalThumb} (cached)`);
          } else {
            console.log(`[projects] Thumbnail for ${repo.name}: placeholder`);
